...
namespace TeacherReward.Controllers
{
	public class AdminController : Controller {
		...
		[HttpGet]
		public ActionResult DoVeto(string ID) {
			int res = Check();
			if (res == 1) {
				return RedirectToAction("Index", "Login");
			} else if (res == 2) {
				...
				TeacherRewardEntities db = new TeacherRewardEntities();
				VetoViewModel this_unfortunate_teacher = new VetoViewModel();
				this_unfortunate_teacher.user = db.Users.Where(x => x.ID == ID).FirstOrDefault();

				return View(this_unfortunate_teacher);
			}
		}
		...
		[HttpPost]
		public ActionResult UnDoVeto(string ID) {
			TeacherRewardEntities db = new TeacherRewardEntities();
			var delEntry = db.Veto.Where(x => x.ID == ID).FirstOrDefault();

			try {
				db.Veto.Remove(delEntry);
				db.SaveChanges();
				return RedirectToAction("ShowVeto", "Admin");
			} catch (Exception) {
				return RedirectToAction("Error", "Admin");
